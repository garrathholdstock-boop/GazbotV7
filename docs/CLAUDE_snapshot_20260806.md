...
### ★★★ 2026-08-06 — RUN STATE IS THE PRIMARY DISCRIMINATOR. READ IT FIRST.

`gazbot7/runstate.py` (CLI: `scripts/run_detect.py`) is injected into every tick **above** the
untradeable meter. It reports **STATE ONLY and never flips a switch** — the arm/bench call is Claude's.
Trailing 30m: `|net|>=80pt` + `ER>=0.35` declares a RUN, `ER>=0.30` holds it.

**Why it outranks everything else.** Classifying every trade of Mon 08-03→Thu 08-06 — taken AND missed,
the missed ones RACED target-vs-stop first-touch — by one variable explained essentially all the P&L:
...
A **$39/trade** spread that dwarfs gate, side and time-of-day. The desk sat flat through **18 of the
week's 26 runs**. ⚠ **When RUN STATE and the untradeable meter disagree, RUN STATE WINS** — the meter is a
DAY aggregate diluted by earlier chop; on 08-06 it read 87/100 STAY-OUT through a +297pt run.
⚠ In-sample and LATE (a trailing window caught 20 of 61 aligned entries). Promising, NOT validated.

**★★ THE ASYMMETRY INVERTS INSIDE A RUN.** The old rule below is right in CHOP and expensive in a RUN.
...
changes, but don't THRASH. Asymmetry **IN CHOP ONLY**: wrongly-benched = cheap, wrongly-armed = expensive
→ sit benched one more tick. **INSIDE A DETECTED ALIGNED RUN, BENCHED IS THE EXPENSIVE ERROR.**
