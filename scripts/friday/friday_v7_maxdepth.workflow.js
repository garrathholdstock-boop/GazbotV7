...
phase('Desks')
await parallel([
  () => agent(`Read ${SCOPE} Part 1. Write the LIVE DESK section (this week's P&L of the 6-gate tournament, per-gate cards, the whippy trend/chop days, the STOP_UNFILLED self-heals, the direction-router trial, day-type splits). ${STYLE} Write to ${SEC}/part1_live.html`, { phase: 'Desks', label: 'part1:live' }),
  () => agent(`Read ${SCOPE} Part 2. Write the SHADOW DESK + PROMOTION section. Run scripts/abs_veto_robustness.py and present abs_veto_55s's full battery (the Monday promotion case) + rgv_short relegation + the faders keep-with-router-guard. Honest caveats. ${STYLE} Write to ${SEC}/part2_shadow.html`, { phase: 'Desks', label: 'part2:shadow' }),
