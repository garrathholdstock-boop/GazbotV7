...
def test_tournament_full_6_roster_even_when_empty(tmp_path):
    ...
    # ★2026-08-15 nipc retired — the board must track the LIVE roster, and showing a retired gate
    # as "benched" implies the router might re-arm it, which is the misleading label web.py's own
    # comment warns about. Its 49 historical trades still render in the blotter.
...
def test_promotion_maps_family_and_flags_candidates(tmp_path):
    ...
    # ★2026-08-15 nipc retired from the roster — live_families is derived from the live slate,
    # so it drops out here automatically. That is the correct coupling: the promotion board
    # should never advertise a family the desk can no longer trade.
